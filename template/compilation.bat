...
%adrsdcc%sdcc -c -mz80 --peep-file peep-rules.txt -I%adrsdk%include %adrsource%/%%~nxi
 )
...
echo ----------------------------------
%adrsdcc%sdcc -o sms.ihx -mz80  --std-c89 --no-std-crt0 --data-loc 0xC000 %adrsdk%lib/crt0_sms.rel %adrsdk%lib/SMSlib.lib *.rel rel/*.rel
...
echo --------------------------------------
%adrihx2sms%ihx2sms sms.ihx %sortie%/%nom%.sms
